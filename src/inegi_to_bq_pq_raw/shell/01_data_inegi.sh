...
echo
echo 'DOWNLOADING INEGI CSV'
curl --retry 20 "$url" -H "${headers[@]}" --data-raw "$data" --compressed > $TEMP_DIRECTORY$INEGI_RAW 2>&- || true
echo 'FILE DOWNLOADED'

echo 
echo 'CLEANING FILE'
awk -F ',' '!/(Defunciones|Total|FUENTE|^$)/ && length($2) > 0 { print }' ${TEMP_DIRECTORY}${INEGI_RAW} | sed -e "s/\r//g" > ${TEMP_DIRECTORY}${INEGI_CLEAN}

echo
head $TEMP_DIRECTORY$INEGI_CLEAN

echo
echo 'MOVING IDM CSV FILE TO GCS'
gsutil -m cp $TEMP_DIRECTORY$INEGI_CLEAN ${BUCKET_PATH}inegi/
